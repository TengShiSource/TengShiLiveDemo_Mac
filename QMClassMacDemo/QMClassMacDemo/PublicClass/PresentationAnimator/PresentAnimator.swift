...
class CBackgroundView: NSView {
   // 重写mouseDown方法,防止事件穿透
...
extension PresentAnimator: NSViewControllerPresentationAnimator{
    func animatePresentation(of viewController: NSViewController, from fromViewController: NSViewController) {
        ...
        // 2. 计算最终显示的frame
        let finalFrame = containerView.frame//NSInsetRect(containerView.bounds, 50, 50)
        // 3. 需要显示的view
        let modalView = viewController.view
        // 4. 设置将要显示视图的初始frame
        modalView.frame = finalFrame
        modalView.setFrameOrigin(NSMakePoint(finalFrame.origin.x+fromViewController.view.bounds.size.width, finalFrame.origin.y))
        // 5 .添加视图到容器视图中
    ...
    func animateDismissal(of viewController: NSViewController, from fromViewController: NSViewController) {
        ...
        NSAnimationContext.runAnimationGroup({ (animationContext) in
            animationContext.duration = 0.5
            viewController.view.animator().setFrameOrigin(NSMakePoint(startFrame.origin.x + fromViewController.view.bounds.size.width, startFrame.origin.y))
        }) {
        // 3. 动画完成后,移除子视图
